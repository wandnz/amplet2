...
int initialise_asn_socket(char *path) {
    ...
    Log(LOG_DEBUG, "Creating local socket at '%s' for asn resolution", path);
    ...
    if ( (sock = socket(AF_UNIX, SOCK_STREAM, 0)) < 0 ) {
        Log(LOG_WARNING, "Failed to open local socket for asn resolution: %s",
                strerror(errno));
        return -1;
    }

    if ( bind(sock, (struct sockaddr*)&addr, sizeof(addr)) < 0 ) {
        Log(LOG_WARNING, "Failed to bind local socket for asn resolution: %s",
                strerror(errno));
        return -1;
    }
     ...
     */
    if ( listen(sock, MAX_ASN_SOCKET_BACKLOG) < 0 ) {
        Log(LOG_WARNING,
                "Failed to listen on local socket for asn resolution: %s",
                strerror(errno));
        return -1;
    }

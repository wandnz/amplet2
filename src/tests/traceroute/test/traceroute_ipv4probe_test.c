 ...
 */
static void check_ipv4_probe(void *packet, uint16_t size, int id, int ttl,
...
int main(void) {
    char packet[MAXLEN];
    uint16_t id, ttl, size, ident, dest, coded_id;
            ...
            for ( ident = 0; ident < IDENT_COUNT; ident++ ) {
                for ( size = 0; size < SIZE_COUNT; size++ ) {
                    for ( dest = 0; dest < DEST_COUNT; dest++ ) {
                        ...
                        length = amp_traceroute_build_ipv4_probe(packet,
                                packet_sizes[size], coded_id, ttls[ttl],
                                idents[ident], &addr);

                        /* check the constructed probe packet */
                        assert(length == packet_sizes[size]);
                        check_ipv4_probe(packet, packet_sizes[size], coded_id,

...
static struct tcpinfo_result_t *get_tcp_info(int sock_fd, int wait_finished) {
    ...
    */
    ...
    /*
     * Core part of a struct that contains the values we care about dont change
...
/*
 * Decission tree clasifer created from dataset generated by this program
 */
int classify(float CoV, float NormRange) {
...
/* 
 * Infer the kind of congestion that is occouring from the collected round trip
 * times of the slow start period by calculating the Normilized range
 * and the Coefficient of variation.
 */
static int rttcount = 0;
void process_rtt(struct test_result_t *res, int32_t *samples) {
    ...
    if ( rttcount <= 2 ) {
        /*
         * Insufficent records to say what form of congestion,
        ...
        /* 
         * somehow managed to not get a single retransmit,
         * link is too healthy to infer congestion reason
         * (or even if congestion is occouring at all)
         */
        res->tcpinfo->congestion_type = 
    ...
    mean = (float)mean_sum / (float)rttcount;
 ...
 */
int writeBuffer(int sock_fd, void *data, size_t length, int32_t *samples) {
    int result;
    ...
    do {
        ...
        if (tcpi->total_retrans == 0 && rttcount < MAXSAMPLES) {
            samples[rttcount] = tcpi->rtt;
            rttcount++;
        }
        free(tcpi);
...
int sendStream(int sock_fd, struct test_request_t *test_opts,
    ...
    int32_t *rtt_samples = calloc(MAXSAMPLES, sizeof(int32_t));
    rttcount = 0;

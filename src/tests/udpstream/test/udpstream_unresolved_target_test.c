...
int main(void) {
    ...
    result = run_udpstream(0, NULL, 1, &target);
    ...
    assert(item->has_direction);
    assert(!item->rtt);
    assert(!item->jitter);
    assert(!item->percentiles);
    assert(!item->has_packets_received);
    assert(item->n_loss_periods == 0);
    assert(!item->has_loss_percent);
    assert(!item->voip);
